...
void print_gen_statistics(tree **population, double duration) {
    ...
    uint64_t gpops = (uint64_t)(calculate_stats(population) / duration);
    Stats *s = run.stats;
...
int evolve() {
    ...
    run.best_ind_gen = calc_pop_fit(population);
    ...
    for(run.cur_gen = 1; run.cur_gen < run.generations; run.cur_gen++) {
        ...
        for(int ind = elitism_n; ind < run.pop_size; ++ind) {
            ...
            while ((depth > run.allowed_depth.max || depth < run.allowed_depth.min) && retries < run.max_retries) {
                ...
                if(rng < run.cross_rate) {
                    tree *parent2 = tournament(population);
                    printfd("Parent 2: ");
        ...
        if (ENABLE_CACHE) {
            build_cache(newt, run.cur_gen);
            if (PDEBUG) printf("Building cache!\n");
        }
        gen_start = clock();
        run.best_ind_gen = calc_pop_fit(new_pop);
        gen_duration = ((double) (clock() - gen_start)) / CLOCKS_PER_SEC;
        print_gen_statistics(new_pop, gen_duration);
